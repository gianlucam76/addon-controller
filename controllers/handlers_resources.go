...
func postProcessDeployedResources(ctx context.Context, remoteRestConfig *rest.Config, remoteClient client.Client,
	...
	if isPullMode {
		setters := prepareSetters(clusterSummary, profileRef)
...
func undeployResources(ctx context.Context, c client.Client,
	...
	if isPullMode {
		err = pullModeUndeployResources(ctx, c, clusterNamespace, clusterName, applicant, logger)
		if err != nil {
			return err
		}
	} else {
...
func pullModeUndeployResources(ctx context.Context, c client.Client,
	clusterNamespace, clusterName, applicant string, logger logr.Logger) error {

	// discard all previous staged resources. This will instruct agent to undeploy
	err := pullmode.RemoveDeployedResources(ctx, c, clusterNamespace,
		clusterName, configv1beta1.ClusterSummaryKind, applicant, string(libsveltosv1beta1.FeatureResources), logger)
	if err != nil {
		return err
	}
	...
	status, err := pullmode.GetResourceRemoveStatus(ctx, c, clusterNamespace,
		clusterName, configv1beta1.ClusterSummaryKind, applicant, string(libsveltosv1beta1.FeatureResources), logger)
	if err != nil {
		return err
	}
	...
	return pullmode.TerminateDeploymentTracking(ctx, c, clusterNamespace, clusterName, configv1beta1.ClusterSummaryKind,
		applicant, string(libsveltosv1beta1.FeatureResources), logger)
}
...
func resourcesHash(ctx context.Context, c client.Client, clusterSummary *configv1beta1.ClusterSummary,
	...
	for i := range clusterSummary.Spec.ClusterProfileSpec.PolicyRefs {
		...
		namespace := libsveltostemplate.GetReferenceResourceNamespace(
			clusterSummary.Namespace, reference.Namespace)
